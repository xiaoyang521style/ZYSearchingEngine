...
+ (NSString *)formatHanyuPinyinWithNSString:(NSString *)pinyinStr
    ...
    if (ToneTypeWithoutTone == [outputFormat toneType]) {
        ...
        pinyinStr = [ZYPinyinFormatter convertToneNumber2ToneMarkWithNSString:pinyinStr];
    }
...
+ (NSString *)convertToneNumber2ToneMarkWithNSString:(NSString *)pinyinStr {
    ...
    if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]?"]) {
        ...
        if ([lowerCasePinyinStr matchesPatternRegexPattern:@"[a-z]*[1-5]"]) {
            NSInteger tuneNumber = [ZYPinyinFormatter getNumericValue:[lowerCasePinyinStr characterAtIndex:lowerCasePinyinStr.length -1]];
            NSInteger indexOfA = [lowerCasePinyinStr indexOfInt:charA];
            NSInteger indexOfE = [lowerCasePinyinStr indexOfInt:charE];
            ...
            if ((defautlCharValue != unmarkedVowel) && (defautlIndexValue != indexOfUnmarkedVowel)) {
                NSInteger rowIndex = [allUnmarkedVowelStr indexOfInt:unmarkedVowel];
